...
**Anime DB** — настольное приложение для ведения **домашней коллекции аниме**.
Технически это проект на **Symfony 2.7 / PHP**: при запуске он поднимает
небольшой **локальный веб-сервер** на порту **56780**, а вы работаете с ним через
обычный браузер по адресу `http://localhost:56780/`. Вся коллекция хранится в
файле базы **SQLite** — никаких внешних серверов не требуется.

Приложение предназначено **только для домашнего использования**. По умолчанию оно
доступно с самого компьютера и из локальной сети (это можно настроить).

Репозиторий — это **ядро** приложения. Сам каталог аниме и «наполнители»
(источники данных: AniDB, Shikimori, World-Art) подключаются как отдельные
плагины-компоненты `anime-db/*`. Ядро отвечает за инфраструктуру: установку и
обновление плагинов, самообновление приложения, миграции базы и автонастройку.

**Как запустить (коротко):**
- Linux: `./AnimeDB start` (остановить — `./AnimeDB stop`).
- Windows: запустить `AnimeDB.exe` (монитор в системном трее).
- Затем открыть в браузере `http://localhost:56780/`.

> 📚 Техническая документация и результаты аудита — в каталоге [`docs/`](docs/)
> (на русском): [обзор аудита](docs/AUDIT.md), [структура проекта](docs/PROJECT.md),
> [техническая часть](docs/TECHNICAL.md), [баги](docs/BUGS.md),
> [рекомендации](docs/RECOMMENDATIONS.md).

Полная инструкция по установке и запуску — ниже (на английском).
